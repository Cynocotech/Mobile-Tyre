...
<body class="antialiased min-h-screen transition-colors duration-200">
    ...
    <div class="max-w-2xl mx-auto px-4 py-5">
      <div class="flex items-start justify-between">
        ...
        <div class="flex items-center gap-2">
          <button type="button" id="btn-refresh-jobs" class="p-2.5 rounded-full app-border border app-text-muted hover:app-text transition-colors" aria-label="Refresh jobs" title="Refresh jobs">
  ...
  (function() {
    ...
    document.getElementById('btn-refresh-jobs').addEventListener('click', function() {
      var btn = this;
      btn.disabled = true;
      loadJobs();
    ...
    if (typeof EventSource !== 'undefined') {
      ...
      evtSrc.onerror = function() {
        evtSrc.close();
        if (!pollInterval) pollInterval = setInterval(loadJobs, 30000);
      };
    } else {
      pollInterval = setInterval(loadJobs, 30000);
    }
  })();
  </script>
  <style>
