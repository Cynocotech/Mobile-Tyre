<?php
/**
 * Admin products API – CRUD for tyres, parts, etc.
 * Uses products table when useDatabase, else database/products.json.
 */
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
  if (function_exists('useDatabase') && useDatabase() && function_exists('dbGetProducts')) {
    echo json_encode(dbGetProducts());
  } else {
...
if (function_exists('useDatabase') && useDatabase() && function_exists('dbSaveProduct')) {
  switch ($action) {
    case 'save':
      $p = [
        'id' => trim($input['id'] ?? ''),
      ...
      if (dbSaveProduct($p)) echo json_encode(['ok' => true, 'product' => $p]);
      else { http_response_code(500); echo json_encode(['error' => 'Failed to save']); }
      exit;
    case 'delete':
      $id = trim($input['id'] ?? '');
      if (!$id) { http_response_code(400); echo json_encode(['error' => 'id required']); exit; }
      if (dbDeleteProduct($id)) echo json_encode(['ok' => true]);
      else { http_response_code(500); echo json_encode(['error' => 'Failed to delete']); }
      exit;
    default:
